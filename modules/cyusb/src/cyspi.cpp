    ...
    CY_SPI_CONFIG* spiConfig)
{
  ...
  rStatus = libusb_control_transfer(devHandle, bmRequestType, bmRequest, wValue, wIndex, (unsigned char*)&localSpiConfig, wLength, ioTimeout);
  if (rStatus == CY_SPI_CONFIG_LEN)
  {
    ...
    spiConfig->protocol = static_cast<CY_SPI_PROTOCOL>(localSpiConfig.mode);
    ...
    uint32_t ioTimeout)
{
  ...
  libusb_fill_bulk_transfer(readTransfer, devHandle, device->inEndpoint, readBuffer->buffer, readBuffer->length, spi_read_cb, &readCompleted, ioTimeout);
  ...
  while (readCompleted == 0)
  {
    ...
    if (r < 0)
    {
      ...
      return static_cast<CY_RETURN_STATUS>(r);
    }
  }
    ...
    uint32_t ioTimeout)
{
  ...
  startSpiTick(true);
  rStatus = libusb_bulk_transfer(devHandle, device->outEndpoint, writeBuffer->buffer, writeBuffer->length, (int*)&(writeBuffer->transferCount), newIoTimeout);
