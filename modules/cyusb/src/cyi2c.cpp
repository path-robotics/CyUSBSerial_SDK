    ...
    CY_I2C_CONFIG* i2cConfig)
{
  ...
  rStatus = libusb_control_transfer(devHandle, bmRequestType, bmRequest, wValue, wIndex, (unsigned char*)&localI2cConfig, wLength, ioTimeout);
    ...
    UINT32 ioTimeout)
{
  ...
  if (pthread_mutex_trylock(&device->readLock) == 0)
  {
    ...
    startI2cTick(false);
#endif
    rStatus = libusb_bulk_transfer(devHandle, device->inEndpoint, readBuffer->buffer, readBuffer->length, (int*)&readBuffer->transferCount, ioTimeout);
#ifdef CY_I2C_ENABLE_PRECISE_TIMING
    ...
    UINT32 ioTimeout)
{
  ...
  if (pthread_mutex_trylock(&device->writeLock) == 0)
  {
    ...
    startI2cTick(true);
#endif
    rStatus = libusb_bulk_transfer(devHandle, device->outEndpoint, writeBuffer->buffer, writeBuffer->length, (int*)&(writeBuffer->transferCount), ioTimeout);
#ifdef CY_I2C_ENABLE_PRECISE_TIMING
...
CY_RETURN_STATUS waitForNotification(CY_HANDLE handle, UINT16* bytesPending, UINT32 ioTimeout)
{
  ...
  libusb_fill_interrupt_transfer(transfer, devHandle, device->interruptEndpoint, i2cStatus, length, i2c_notification_cb, &transferCompleted, ioTimeout);
  ...
  if (transfer->status == LIBUSB_TRANSFER_COMPLETED)
  {
    ...
    if (i2cStatus[0] & 0x80)
    {  //Error notification is for write
      if ((i2cStatus[0] & CY_I2C_ERROR_BIT))
      {
